...
none <- vector(mode='character', length=0) # It would be nice to use NULL or NA
...
df<-mobius_run_with(c('Evapotranspiration adjustment', 'Baseflow index'),
                    list(none, none),
                    c(2.0, 0.7),
...
# This is the predicted flow. No idea how to get the observed flow!
flow <- mobius_get_result_series('Reach flow (daily mean)', c('UpperTweed'))
